...
\begin{sectionbox}
	...
	\textbf{For 2 Gaussians}, with $\ma C_0 = \ma C_1= \ma C \implies \log R(\vec x) = \vec w^\top \vec x - w_0$:\\
	$\vec w^\top = (\vec {μ}_1 - \vec{μ}_0)^\top \ma C$\quad (Normal Vector)\\
	$w_0 = \frac{(\vec {μ}_1 - \vec{μ}_0)^\top \ma C (\vec {μ}_1 - \vec{μ}_0)}{2}$\quad (Constant Translation)\\
	$\implies T: \mathbb X \ra \R, \vec x \mapsto \begin{cases} 1 & \vec w^\top \vec x - w_0 > 0 \\ 0 & \text{otherwise}\end{cases}$ (Affine Decision Rule)
...
(vielleicht noch was davon übernehmen)
